...
public class Utils
{
	...
	public static void CallMethod(object src, string methodName, object[] args)
	{
		...
		{
			Debug.LogError( string.Format( "CallMethod(): Method not found: {0}.{1}" , src.GetType() , methodName ) );
		}
	}
	...
	static public string GetFileNameFromFleetName( string name , string fileType )
	{
		...
		{
			var bit = new System.Text.StringBuilder();
			{
				bit.Append( nameBit.ToLower() );
				bit.Replace( "'" , string.Empty );
				bit.Replace( "\"" , string.Empty );
				bit.Replace( "," , string.Empty );
				bit.Replace( "." , string.Empty );
			}
			output.Append( string.Format( "{0}_" , bit ) );
		}
		output.Remove( output.Length-1 , 1 );
		output.Append( string.Format( ".{0}.json" , fileType ) );
	...
	static public string LoadJsonFromFile(string filename)
	{
		string text;
		{
			string path = string.Format( "{0}/Resources/Data/{1}.txt" , Application.dataPath , filename );
			if( File.Exists( path ) )
			{
			...
			{
				var textAsset = Resources.Load( string.Format( "Data/{0}" , filename ) )  as TextAsset;
				if( textAsset==null )
				{
	...
	static public bool SaveJsonToFile(string filename, string jsonString)
	{
		...
		Debug.Log( string.Format( "Wrote to {0}" , filename ) );
		return true;
	}
	...
	static public string toOrdinal(int number) {
		...
		return string.Format( "{0}{1}" , number , ending );
	}
...
def decToRoman(num,s,decs,romans):
	...
	*/
	...
	static public void TranslateUVs( GameObject go , Vector2 offset )
	{
		...
		for( int i=mesh.uv.Length-1 ; i!=-1 ; i-- )
		{
			Vector2 v = default(Vector2);
			v.x = mesh.uv[i].x + offset.x;
			v.y = mesh.uv[i].y + offset.y;
			newUVs[i] = v;
		}
	...
	static public void SetUVs ( GameObject go , Vector2 offset )
	{
		var meshFilter = go.GetComponent<MeshFilter>();
		Mesh mesh = meshFilter.mesh;
	...
	static public void TranslateWeaponUVs ( GameObject go , Vector2 offset )
	{
		var meshFilter = go.GetComponent<MeshFilter>();
		Mesh mesh = meshFilter.mesh;
	...
	public static Texture2D LoadTextureFromFile ( string path )
	{
        ...
        else
        {
            Debug.LogError(
				string.Format(
					"Reading preview file failed! Make sure JSON file is filled properly and/or file exists: \"{0}\"",
					path
				)
			);
            return null;
	...
	{
		var moveTargets = new Stack<Transform>();
		moveTargets.Push( root );
		while( moveTargets.Count!=0 )
		{
			var currentTarget = moveTargets.Pop();
	...
	public static void PrepareVisualPrefab ( GameObject go )
	{
		go.transform.localScale = Vector3.one;
		SetLayer( go.transform , LayerMask.NameToLayer("GUI") );
